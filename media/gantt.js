...
(() => {
  ...
  function setupKeyboard(ctx) {
    ...
    addDocListener("keydown", (e) => {
      ...
      if (modKey && e.key === "z" && !e.shiftKey) {
        ...
        e.preventDefault();
        const blockedBars = Array.from(document.querySelectorAll(".issue-bar[data-issue-id]")).filter((bar) => bar.querySelector(".blocker-badge"));
  ...
  function initializeGantt(state) {
    ...
    function showDeletePicker(x, y, relationId, fromId, toId) {
      ...
      deleteBtn.textContent = "\u{1F5D1}\uFE0F Delete relation";
