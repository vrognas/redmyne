...
### 54. [x] src/redmine/models/time-entry.ts:11 — TimeEntry.hours typed `string` but Redmine GET responses return a number — type lie forces casts and invites string-concat bugs
...
### 63. [x] src/webviews/gantt/gantt-render-types.ts:31 — GanttRenderContext carries ~12 fields no generator ever reads, plus 3 exported interfaces with zero importers
...
### 68. [x] src/utilities/issue-picker.ts:1 — issue-picker.ts is a 1371-line god-file mixing search engine, four caching layers, and two full QuickPick UIs
...
- **status:** Done (search-engine-only scope, 2026-06-18). Extracted the vscode-free search engine + its two search caches to src/utilities/issue-search.ts (searchIssuesWithFuzzy / fuzzyFilterIssues / parseSearchOperators / getOrCreateFuse / buildProjectPathMap), now exported and unit-tested directly in test/unit/utilities/issue-search.test.ts (the __testIssuePicker hack shrank to UI/data helpers). Drift bug fixed in a separate commit: pickIssue's search-time tracking check now routes through the fail-open getTimeTrackingStatusCached. Public API unchanged (7 consumers untouched). Deferred: the 3 data caches (myIssues/timeTracking/projectPath) + accessors stay in issue-picker.ts; the three-QuickPick-UI item-construction dedup (#97) is untouched.
...
### 81. [x] src/kanban/kanban-commands.ts:511 — Timer keeps ticking through logEarly/deferTime dialogs: completion can fire mid-flow, double-logging time to Redmine
...
### 89. [x] src/commands/configured-command-registrar.ts:46 — Registrar's overloaded (withPick|node, props, ...args) wire protocol leaks into every caller and silently drops primitive args
...
### 136. [x] src/controllers/domain.ts:31 — Domain IssueStatus duplicates models/common IssueStatus, forcing a parallel getIssueStatusesTyped API and ad-hoc conversion
...
### 138. [x] src/utilities/issue-picker.ts:195 — Issue label template '#id subject' (with icon/closed/disabled variants) repeated 25+ times across 9 files with inconsistent separators
...
- **status:** Added src/utilities/issue-label.ts (pure, no vscode) + migrated 23 picker/tree/notification sites across 6 files; each site's existing icon/separator preserved (no visible change). Deferred: timesheet/index.js ×2 (webview bundle — folds into #52); gantt-html-generator + markdown-escaped tooltip sites left inline (raw-text helper would double-escape pre-escaped subjects); separator canonicalization (a visible UX change) intentionally not done in this sweep.
