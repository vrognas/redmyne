...
## Webview bundles (media/)

- `media/{gantt,timesheet}.{js,css,js.map}` are **generated by esbuild and gitignored** — do NOT commit them. `vscode:prepublish` (and the CI release job) run `npm run compile` before packaging, so the bundle that ships is always rebuilt from `src/webviews/**`; the working copy is build output only.
- `npm run watch` (dev profile, unminified) rebuilds and overwrites these files on save — harmless now that they're untracked. (F5 "Launch Extension" has no preLaunchTask, so it does not rebuild; run watch separately or `npm run compile` for a production bundle.)
- Only the **vendored** media assets are tracked: `flatpickr*`, `webview-common.css`.
